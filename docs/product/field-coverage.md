...
| -------- | ------------------- | ---------------------------------------------------------------------------- | ----------------------------------------- | -------- |
| 首页     | Hero 背景与首屏指标 | 静态视觉配置，不接入 `getBanners()`                                          | 使用固定 Hero 背景，不被 banner 图片覆盖  | 暂不展示 |
| 首页     | banner 图片/动作    | `getBanners().data.list.title/subtitle/image_url/action_url`                 | 内容区展示 banner 轮播；不展示标题/副标题 | 已覆盖   |
