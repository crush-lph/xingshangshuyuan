...
| 电子票       | 报名记录、参会人、状态           | `getUserEvents()`                                    | 根据报名记录ID展示已报名活动；二维码接口缺失时提示 | 部分覆盖 |
| 电子票       | 二维码、核销码                   | 当前 OpenAPI 未提供电子票二维码接口                  | 不调用旧接口，不生成假二维码                       | 接口待补 |
...
| 我的活动     | 活动报名记录/电子票 | `getUserEvents()`                                     | 展示真实报名记录；有报名记录ID时进入票券状态页 | 已覆盖   |
...
| 后台核销     | 当前真实接口        | `getEvents()`、`getEventDetail()`                     | 展示真实活动；新版 OpenAPI 未提供核销动作      | 部分覆盖 |
| 后台核销     | 电子票核验结果      | 当前 OpenAPI 未提供扫码核销接口                       | 不展示扫码核销操作，不调用旧接口               | 接口待补 |
