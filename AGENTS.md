...
- 页面和卡片严禁被内容撑出屏幕宽度。所有页面主内容、卡片、列表、统计栅格、横向筛选区必须有明确的宽度收缩约束，例如 `max-w-full`、`min-w-0`、`overflow-hidden`、`break-all`、`truncate` 等，保证微信小程序里不会出现横向滚动或右侧内容被裁出屏幕。
- 横向 `ScrollView` 只能用于内部可滚动内容，外层必须限制为 `w-full max-w-full overflow-hidden`；内部列表不要用大额左右 padding（例如 `px-[220rpx]`）制造居中效果，否则会把页面整体撑宽。需要留白时只使用小额 `pr-1`、`gap` 或容器内对齐。
- 长标题、描述、价格、地址、时间、标签等接口返回文本必须假设不可控。放在 `flex`/`grid` 中时，父项加 `min-w-0`，文本按场景使用 `break-all` 或 `truncate`，不允许依赖默认文本行为。
- 新增或重构页面后必须检查 375px 级手机宽度下是否存在横向溢出，尤其是筛选胶囊、统计卡、列表卡片、按钮组和长接口文案。发现溢出时修布局约束，不要通过扩大容器宽度规避。
